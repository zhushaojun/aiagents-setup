...
## 1.1 为什么变量名必须是 `NEWAPI_KEY`（下划线）

这是实测结论，不是洁癖：

- **PowerShell**：`$env:NEWAPI-KEY = "x"` 直接**语法报错**（`意外的标记"-KEY"`），必须写成 `${env:NEWAPI-KEY}` 才能用；
- **Linux / Git Bash**：`export NEWAPI-KEY=x` 是**非法标识符**，根本设不进去，`$NEWAPI-KEY` 也读不到。

`NEWAPI_KEY` 在 Windows、Linux、PowerShell、Bash 里都好用，所以统一用下划线。
...
下面这份清单是**实测结果**（用我们的 Key 直接查中转的模型列表接口得到，2026-09）：
...
| 工具 | 主力 | 备选 | 理由 |
| --- | --- | --- | --- |
| Codex | `gpt-6-sol` | `gpt-6-luna`、`gpt-5.6-sol` | Codex 与 GPT 系列同源，工具调用最稳 |
...
| `503 model_not_found` / `No available channel for model` | 模型名写错，或该模型不在清单里 | 按第 4 节清单核对 |
