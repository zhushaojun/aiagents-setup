...
| `--list-models` 里没有 newapi 的模型 | `$NEWAPI_KEY` 没生效（环境变量没设或没新开终端）；确认 `$env:NEWAPI_KEY.Length` 是 51。无密钥时 pi 还会对 `settings.json` 的每条 `enabledModels` 报 `Warning: No models match pattern "newapi/..."` |
| 报 `No API key found for newapi.` | 同上，是密钥没解析到。**别按提示去 `/login`**：那会把明文密钥写进 `auth.json`，而它的优先级高于 `models.json` 的 `apiKey`，之后改环境变量都不生效 |
| 曾经 `/login` 过，改了环境变量却不生效 | `auth.json` 里的旧密钥盖住了 `$NEWAPI_KEY`。删掉其中的 `newapi` 条目，命令见[附录](10-appendix-full-config.md) 3.2 |
