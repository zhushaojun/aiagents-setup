...
- **文件名用 `NN-kebab-case`**：`NN` 两位数字就是推荐阅读顺序（`00` 总览 → `10` 附录），名字用英文小写连字符（如 `05-claude-code.md`），文档 H1 标题保持中文。`README.md` / `AGENTS.md` 不编号。新增一篇教程时，取一个未被占用的数字，并同步更新 README 第 3 节的文档地图。
...
- **缺图的位置用 HTML 注释占位**：`<!-- TODO 截图：要拍什么 -->`。用 `rg 'TODO 截图'` 找出全部待补截图。
