...
- **定位**：Anthropic 官方客户端，最先跑通"命令行 AI 编程"这条路的工具，目前版本迭代极快（我们写这份文档时是 2.1.x）。
...
- **我们自己的实测**：把一张 8×8 的小 PNG 直接附在提示里（`claude -p "这张图是什么颜色 @图片路径"`），返回 **"红色"**，`is_error=false`。→ **直接附图是可以用的。**
- **外部独立对照实测**（2026 年，同一张图、10 款模型）：
...
一句话：**不是我们的中转不行（中转的图片接口实测正常，`claude-opus-5`、`deepseek-v4.1-flash` 都能收图），是 Claude Code 这个客户端在第三方模型上把图片这条路做坏了。**
...
| VS Code 扩展里模型不对 | 扩展与命令行共用配置；先在命令行 `claude -p "只回复两个字：可用"` 跑通 |
