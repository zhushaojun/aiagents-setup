...
面向**使用指定 NewAPI 服务的 Windows 内部学员**。主线采用 PowerShell 7；服务入口：[NewAPI](https://newapi.ttxs.site)。Linux 步骤作为补充，其他供应商需自行核对协议与凭据。
...
| --- | --- | --- | --- |
| Codex | 0.156.x（精确版本未记录）；正文另有 0.157.0 输出示例，未关联验证日志 | 2026-09-25 | `codex --version` |
| pi | 0.87.x（精确版本未记录；正文 0.87.1 示例不代替验证日志） | 2026-09-25 | `pi --version` |
| Claude Code | 2.1.x（精确版本未记录） | 2026-09-25 | `claude --version` |
| OpenCode | `@opencode/cli` 2.0.x（精确版本未记录） | 2026-09-25 | `opencode --version` |
...
| 日期 | 操作系统及版本 | 客户端及精确版本 | 模型 ID | 协议 | 检查类型 | 结果与通过项目 | 脱敏证据位置 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| 待填写 | 待填写 | 待填写 | 待填写或不适用 | 待填写或不适用 | 静态语法检查／官方资料核对／真实链路测试（三选一） | 通过／失败／未执行，并注明具体项目 | 日志路径或来源链接及章节 |

静态语法检查只验证可解析性；官方资料核对记录来源与核对日期；真实链路测试分别记录请求、读文件、执行命令等结果。本次真实链路测试仍为“未执行”，不以本机当前版本反推历史测试版本，也不把版本输出示例当作已验证组合。证据中不要保存密钥或未脱敏配置。
