...
| --- | --- | --- |
| Windows | 10 / 11 **x64** | — |
| 环境变量 | `NEWAPI_KEY` 已设置（由被启动的 CLI 读取） | `$env:NEWAPI_KEY.Length` → `51` |
| **Git** | 必装（worktree 是 Orca 的核心机制） | `git --version` |
| **至少一个 CLI 智能体** | 已装好并验证通过 | 见下表 |
| 内存 | 8 GB 起步，16 GB 更舒服 | — |

上表最后一行的"验证通过"，指的是你**先在终端里单独跑通**了至少一个智能体：
...
`Settings → Agents` 里有 Claude 和 Codex 的账号列表，还有热切换（hot-swap）功能——那是给**官方订阅**用户薅多账号额度用的。我们的 Codex 走的是 `~/.codex/config.toml` 里的中转配置，而 Orca 管理的**额外账号会使用独立的 home 目录**（`~/.local/share/orca/codex-accounts/<id>/home`），你的 `config.toml` 不会跟着过去。

> **原则**：用中转时，保持 **system default**（即真实的 `~/.codex` / `~/.claude`），不要 `Add account`。

**坑二：Orca 会给智能体预填"跳过权限确认"的参数。**

为了配合"worktree 是一次性目录"的思路，Orca 默认会给支持的 CLI 加上权限绕过参数（Claude Code 是 `--dangerously-skip-permissions`，Codex 是 `--dangerously-bypass-approvals-and-sandbox`）。**这会让智能体在你的项目里放开手脚**。
...
完整走一遍官方那条"加仓库 → 建 worktree → 起智能体"的路径，大概 5 分钟：

## 5.1 添加仓库（Add Repo）

点侧边栏的 **Add Repo**，选一个**本地的 git 仓库目录**（建议先用一个小项目练手）。
...
新建的 worktree 里**没有 `node_modules`**，也**没有 `.env`**。Orca 给了三种办法：
...
> Windows 上的现实建议：`node_modules` 用共享（省磁盘、省时间），`.env` 用 `.worktreeinclude` 复制。第一次建完 worktree 后，**盯一眼 setup 输出**，确认依赖真的装好了。
...
这样每个新 worktree 一建好，依赖就已经就绪，智能体不用等。命令行里也能控制单次行为（`--setup run|skip|inherit`）：
...
| 想彻底重来 | 关掉 Orca，`Settings` 里的 Agent status hooks 关闭，然后按 9 卸载；**你的项目目录和 git 仓库不受影响** |
...
> 你的 **git 仓库本体不会被 Orca 改动**；worktree 只是同一仓库的额外检出，删掉不会影响主检出。
